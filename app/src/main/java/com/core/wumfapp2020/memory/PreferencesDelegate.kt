...
private const val SETTINGS_KEY = "wumf.app"

@Suppress("UNCHECKED_CAST")
open class PreferencesDelegate<TValue>(
    private val name: String,
    private val defValue: TValue
) : ReadWriteProperty<Any?, TValue> {

    val preferences: SharedPreferences by unsyncLazy {
        DynamicApp.instance!!.getSharedPreferences(SETTINGS_KEY, Context.MODE_PRIVATE)
    }

    open override fun getValue(thisRef: Any?, property: KProperty<*>): TValue {
        with(preferences) {
    ...
    open override fun setValue(thisRef: Any?, property: KProperty<*>, value: TValue) {
