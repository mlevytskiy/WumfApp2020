...
import com.core.wumfapp2020.base.languagesdialog.LanguagesAdapter
...
fun showCheckAppIfExistOnGooglePlayDialog(
    context: Context, appContainer: AppContainer, addPkgInMemory: () -> Unit,
    moveToAddToMyCollectionScreen: () -> Unit
): DialogInterface {
    val dialog = createDialogBuilder(context)
        .setView(
            createCheckAppIfExistOnGooglePlayDialogView(
                context,
                appContainer,
                addPkgInMemory,
...
fun showSuccessLoginDialog(
    context: Context, image: TdApi.File?, name: String, surname: String, telegramId: Int?, phoneNumber: String?,
...
fun createSuccessLoginDialogView(
    context: Context, image: TdApi.File?, name: String, surname: String, contactsAmount: Int, telegramId: Int?,
    phoneNumber: String?, allContacts: List<Int>, contactsWithWumf: List<TdApi.User>, dismissDialog: () -> Unit
): View {
...
private fun createCheckAppIfExistOnGooglePlayDialogView(
    context: Context, appContainer: AppContainer, addPkgInMemory: () -> Unit,
    moveToAddToMyCollectionScreen: () -> Unit, dismissDialog: () -> Unit
): View {
    ...
    val viewModel = getAppComponent().checkAppInGooglePlayFactory.create(
        appContainer,
        addPkgInMemory,
        moveToAddToMyCollectionScreen,
        dismissDialog
...
fun showCountriesDialog(context: Context, select: (Country) -> Unit): DialogInterface {
    val dialog = createDialogBuilder(context).setView(createCountriesView(context = context, select = select)).show()
    dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(ContextCompat.getColor(context, R.color.blue))
    return dialog
}

fun showLanguagesDialog(context: Context, select: (String) -> Unit): DialogInterface {
    dialog = createDialogBuilder(context).setView(createLanguagesView(context = context, select = {
        select(it)
        dialog?.dismiss()
    })).show()
        dialog?.getButton(AlertDialog.BUTTON_NEGATIVE)?.setTextColor(ContextCompat.getColor(context, R.color.blue))
...
private fun createCountriesView(context: Context, countries: ArrayList<Country> = ArrayList(), select: (Country) -> Unit): View {
    ...
    if (countries.isEmpty()) {
        val holder = CountriesHolder(context).apply {
            syncLoad()
        }
        countries.addAll(holder.countries)
    }
...
fun createLanguagesView(context: Context, select: (String) -> Unit): View {
    val li = LayoutInflater.from(context)
    val listView = li.inflate(R.layout.dialog_countries, null) as ListView
    val gray = context.resources.getColor(R.color.observatory_alpha_6)
    val languages = context.resources.getStringArray(R.array.languages)
    val adapter = LanguagesAdapter(languages, Color.WHITE, gray)
    listView.adapter = adapter
    listView.onItemClickListener = AdapterView.OnItemClickListener { _, _, position, _ -> select(languages[position]) }
    return listView
}

fun showAppDialogFromMyCollection(
    appContainer: AppContainer,
    context: Context,
    showInGPBlock: () -> Unit,
    removeFromMyCollection: () -> Unit
) : DialogInterface {
    val dialog = createDialogBuilder(context).setView(
        createAppDialogViewFromMyCollection(
            appContainer,
            context,
            showInGPBlock,
...
fun createAppDialogViewFromMyCollection(
    appContainer: AppContainer, context: Context,
    showInGPBlock: () -> Unit, removeFromMyCollection: () -> Unit
): View {
...
fun showAppDialog(
    appContainer: AppContainer, context: Context, showInGPBlock: () -> Unit, showPeopleWhoLikesBlock: () -> Unit,
    peopleWhoLikes: List<Int>
): DialogInterface {
    val dialog = createDialogBuilder(context).setView(
        createAppDialogView(
            appContainer,
...
fun showSimpleDialog(
    context: Context, arrayId: Int, checkedItem: Int, select: (Array<String>, Int) -> Unit, cancel: (DialogInterface) -> Unit,
