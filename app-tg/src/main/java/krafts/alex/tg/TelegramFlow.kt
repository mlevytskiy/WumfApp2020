...
interface FlowProvider {
    ...
    fun send(function: TdApi.Function, exceptionHandler: ((e: Throwable)-> Unit), resultHandler: (TdApi.Object) -> Unit)
}
...
open class TelegramFlow(
    ...
    inline fun <reified UpdateType : TdApi.Update, ResultType>
        ...
        mainFlow.transform { value ->
            if (value is UpdateType) {
                Log.i("testr", "emit ${value.javaClass}" )
                emit(block(value))
            } else {
                Log.i("testr", "emit2 ${value.javaClass}" )
            }
        }
